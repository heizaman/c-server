...
char* indexes(const char* path)
{
    ...
    if (strcmp(&new_path[path_length - 2], "/") == 0)
    {
        html = "index.html";
        php = "index.php";
    }
    ...
    {
        returnpath = malloc(strlen(path) + strlen(html) + 1);
        strcpy(returnpath, path);
        strcat(returnpath, html);
        returnpath[strlen(path) + strlen(html)] = '\0';
        //char* output = returnpath;
        //free(returnpath);
        return returnpath;
    }
    else if(access("index.php", F_OK) != -1 ) 
    {
        returnpath = malloc(strlen(path) + strlen(php) + 1);
        strcpy(returnpath, path);
        strcat(returnpath, php);
        returnpath[strlen(path) + strlen(php)] = '\0';
        //char* output = returnpath;
        //free(returnpath);
        return returnpath;
    }
...
const char* lookup(const char* path)
{
    ...
    char patharr[strlen(path) + 1];
    strcpy(patharr, path);

    char* ext = strrchr(patharr, '.');
    char* extension = malloc(strlen(ext)*sizeof(char));
    for (int i = 1; i < strlen(ext); i++)
    {
        extension[i-1] = ext[i];
    }
...
bool parse(const char* line, char* abs_path, char* query)
{
    ...
    char linearr[strlen(line) + 1];
    strcpy(linearr, line);
    linearr[strlen(line)] = '\0';
    
    char* method;
    char* requesttarget;
    char* httpversion;
    
    method = strtok(linearr, " ");
    requesttarget = strtok(NULL, " ");
    httpversion = strtok(NULL, "\r\n");

    char methodarr[strlen(method) + 1];
    strcpy(methodarr, method);
    char requesttargetarr[strlen(requesttarget) + 1];
    strcpy(requesttargetarr, requesttarget);
    char httpversionarr[strlen(httpversion) + 1];
    strcpy(httpversionarr, httpversion);

    methodarr[strlen(methodarr)] = '\0';
    requesttargetarr[strlen(requesttargetarr)] = '\0';
    httpversionarr[strlen(httpversionarr)] = '\0';
    ...
    if (strcmp(methodarr, "GET") != 0)
    {
        error(405);
        return false;
    }

    if (strncmp(requesttargetarr, "/", 1) != 0)
    {
        error(501);
        return false;
    }

    if (strchr(requesttargetarr, '"') != NULL)
    {
        error(400);
        return false;
    }

    if (strcmp(httpversionarr, "HTTP/1.1") != 0)
    {
        error(505);
        return false;
    }
    ...
    if(strchr(requesttargetarr, '?') == NULL) {
        strcpy(abs_path, requesttargetarr);
        abs_path[strlen(requesttargetarr)] = '\0';
        query[0] = '\0';
    }
    else {

        abs_path_str = strtok(requesttargetarr, "?");
        query_str = strtok(NULL, "\0");

        if (query_str != NULL)
        {
            strcpy(abs_path, abs_path_str);
            strcpy(query, query_str);
     
            abs_path[strlen(abs_path_str)] = '\0';
            query[strlen(query_str)] = '\0';
        }
        else
        {
            strcpy(abs_path, abs_path_str);
